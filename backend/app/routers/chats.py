...
@router.get("/{conversation_id}/seller", response_model=dict)
async def get_chat_seller(
    conversation_id: int,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """
    Get the seller_user_id for a specific conversation.
    """
    try:
        res = await db.execute(select(ChatConversation).where(ChatConversation.conversation_id == conversation_id))
        conv = res.scalar_one_or_none()
        ...
        return {"seller_user_id": conv.seller_user_id}
